...
class Wechat {
    ...
    async getWechatConfig(ctx:Context):Promise<void> {
        const ticket = await wecaht.getJsapi_ticket();
        const noncestr = Math.random().toString(36).substr(2); // 随机字符串
        const timestamp = Math.ceil(new Date().getTime() / 1000);
        const sign = `jsapi_ticket=${ticket}&noncestr=${noncestr}&timestamp=${timestamp}&url=${ctx.request.body.url}`;
        const jsapiTicket = wecaht.encryption(sign);
        ctx.body = {
            code: 0,
            data: {
                appId: config.appId,
